...
Here I will try to explain some things that I consider interesting about the work that it is **currently in the D.E.M.O.**. I planned to keep adding things to this document while implementing them into the game. But most of this content ended up in another project of mine (It_Fights) so check that out if you want! It has some interesting AI stuff :)
...
Lastly, I create a new texture that has the final frame with 4 iterations of a Cone Tap blur applied to it. The original unaltered frame is merged with the blurred one to get a distinct look that simulates the glass of old TV screens and combats some aggressive effects done before.
